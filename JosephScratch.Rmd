...
Long = Long %>% mutate(termCat = cut(gestational_age, c(breaks = c(0, 32, 37,41, 50))), center = as.character(center),
                       ...
                       dde_cut = cut(dde, breaks = 5),
                       pcb_cut = cut(pcb, breaks = 5),
...
##
Long_Score_NA_rm = Long
modordscore = polr(gestord ~ log(dde) + log(pcb) + score_education + score_occupation + score_education + log(triglycerides) + maternal_age + smoking_status + cholesterol + albuminTested, data = Long)
...
modord = polr(gestord~ center+log(dde) + log(pcb)+log(triglycerides) + I(pcb/triglycerides) + maternal_age + smoking_status + cholesterol + albuminTested, data = Long)
summary(modord)
