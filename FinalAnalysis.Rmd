...
In order to assess how exposure to DDE and PCBs relates to the risk of premature delivery we decided to investigate how well the concentration doses of DDE and PCBs predict premature delivery. We noticed that there are clinical categories for gestational age with gestational ages of less than 33 weeks corresponding to severely preterm births, gestational ages of less than 38 weeks corresponding to preterm births, and gestational ages of between 38 and greater corresponding to full- to late-term births, which correspond to guidelines provided by the World Health Organization [@who]. Owing to the relative implausibility of gestational terms exceeding 44 weeks and resulting high likelihood that such observations in the data are erroneous, we opted to exclude observations exceeding 44 weeks. Because the distribution of gestational periods appears to be reasonably symmetric when keeping these large observations, it is possible that the entire distribution has been shifted to the right owing to some bias in gestational age data collection. This is speculative and thus we do not wish to make sweeping changes of on this basis. The natural and ordinal categorization of gestational age leads us to employ ordinal logistic regression (otherwise known as proportional odds logistic regression) as our modeling method [@mccull1980]. This method provides useful interpretation in terms of risk and probability of pre-term birth while using the naturally ordinal structure of the gestational age categories and differentiating between the severity of severely pre-term births and moderately pre-term births. This structure also addresses the non-normality of the response which is readily observable and impedes the use of standard linear regression. The model has the following structural form:

$$ logit(Y_i\leq j) = \alpha_j - \beta^T X_i $$
From which pme derives:
...
The first and fourth of these requirements are evidently satisfied. The non-multi-collinearity requirement is addressed by performing a PCA analysis of the pcb_* predictors, which are heavily related and exhibit relatively high correlations. The proportional odds assumption may be tested via the Brant test for which we have obtained satisfactory results. We note that is a high degree of missingness for certain predictors, such as the "Score" variables. We use the MICE package in R to impute these missing data points. MICE generates multiple imputed datasets, as imputation is a random process, and then pools the regression results from these imputed data sets to provide one set of results. We note in our EDA the relatively high collinearity among the pcb_* predictors. In order to deal with this, we perform a principle components analysis (PCA) on these predictors in order to summarize the data as new predictors that are uncorrelated. We end up concluding that the first principle component is sufficient. As the weights in the first principle component are all of the same sign and of broadly similar magnitude, we could have perhaps simply taken the sum of the pcb_* predictors as an alternative methodology. We note that there is one observation with missing pcb_* data. We remove this observation as its impact is de minimis. Finally, a key goal of our efforts is to determine whether there is a residual dose reponse of DDE an PCB after accounting for other predictors, such as blood cholesterol/triglyceride levels, as well as various metrics capturing socioeconomic status and lifestyle.
