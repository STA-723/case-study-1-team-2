...
- id: mccull1980
  ...
  issued:
    year: 1980
    month: 2
abstract: We examine a data set from the Collaborative Perinatal Project to assess the degree to which the likelihood and severity of pre-term births are influenced by the level of concentration of chemicals DDE and PCB in the blood of expecting mothers. We find positive evidence of an effect, using an ordinal logistic regression model to assess risk of pre-term births while also differentiating among different severities of pre-term births. We provide a summary of our findings as well as a discussion of the advantages and drawbacks our methods, as well as of potential alternatives.
...
- Predictors are either categorical, ordinal, or continuous

The first requirement is evidently satisfied by the natural ordering of preterm birth categories. Performing a Principal Component Analysis (PCA) of the PCB predictors, which exhibit relatively high correlations (\ref{fig:gpairs} (right)), to produce new uncorrelated variables, guarenteeing the multi-collinearity assumption. The proportional odds assumption may be tested via the Brant test for which we obtain satisfactory results [@brant1990]. We note that "Score" variables have a high degree of missingness and use the MICE package in R to impute these missing data points. MICE generates multiple imputed datasets, as imputation is a random process, then pools the regression results from these imputed data sets to provide one set of results. We remove one observation with missing PCB data. We also include other potential confounders such as blood cholesterol/triglyceride levels, the center where these measurements were taken and metrics capturing socioeconomic status and lifestyle in the model. Finally, we perform model selection using F-tests.
...
Many of the variables in the dataset exhibit a variety of issues such as measurement errors, zero-inflation, multi-collinearity and missingness. Gestational age potentially contain measurement errors since its values range from 27 to 90 (Figure \ref{fig:gesthist}). As the date of conception typically relies on the first day of the mother's last menstrual period, if the mother missed any menstrual cycles, the estimated gestational age could be more than 44 weeks, normally considered as the upper bound of a plausable late term gestation. We address this issue by keeping observations with 44 weeks or less in gestational age.

We also see an inflation at zeros in the PCB variables due to the limitations of measurement equipment that round measurements below a certain threshold to zero. Consequently, we add half of the smallest value of PCB measurements to the zero and log-transform these variables. As seen in Figure \ref{fig:gpairs} (right), the PCB variables are highly correlated. We use PCA to create uncorrelated variables summarizing the variation in the PCB variables. Among the other covariates, only DDE and the first principal component of PCBs, triglycerides and cholesterol are mildly correlated (Figure \ref{fig:gpairs} left).

Missigness is most significantly present in the variable albumin for which less than 10% of women had their levels recorded. Since albumin can be an indicator of liver disease, kidney disease, and possibly gestational diabetes, being tested for albumin can suggest the mother's health condition. Figure \ref{fig:albumin} shows that being tested for albumin seems to associate with slightly larger gestational age. 

Bivariate plots of gestational age against each predictor show associations between premature birth and predictors such as race, centers, a quadratic function of maternal age, concentration of DDE and cholesterol (Figure \ref{fig:bivartrend}). 
...
```{r gesthist, fig.cap="\\label{fig:gesthist}Distribution of gestational age ranges from 27 to 90 weeks"}
ggplot(longnecker_base, aes(x = gestational_age)) + geom_histogram(bins = 50)
```


```{r bivartrend, fig.show="hold", out.width="50%", fig.cap="\\label{fig:bivartrend}Bivariate plots of gestational age against predictorss"}
...
The assumption holds at the 0.05 level.

```{r, diagnostics-prop-odds, results='hide'}
library(brant)
...
Before we discuss our main results, we will touch on a number of F-tests that we performed to help determine the appropriate structure and complexity of our model:
...
We make a couple of comments on the above. We had hypothesized that we could conclude a triglyceride-DDE/PCB interaction owing to the fat-solubility of the DDE an PCB compounds. Our finding for the inclusion of a quadratic maternal age term is in line with our observations in the EDA of probability of pre-term birth versus maternal age. Taking into account these results, we express our final model form below in R-standard notation:
...
Testing inclusion of both DDE and PCB vs. control model gives p=`r round(control.p[2],3)`. This is a test of the inclusion of both DDE and PCB vs the exclusion of both. This indicates that the addition of DDE and PCB as predictors represents a substantial improvement in explanatory power of pre-term birth risk over control, including explanatory value in differentiating among degrees of severity in pre-term birth. Marginally in the full model each is at the edge of significance (at the 5% level), likely due to their positive correlation:
...
```
We note the following interpretations of the above:
...
- For 10% increase in the first PC summary of PCBs, the odds of having more normal gestational age decreases by `r round(100 - 100 * exp(.1*inter_table[2,1]),1)`%. See \@ref(fig:Results) below fora pictoral representation of fitted dose response probabilities across gestational term severities. 
...
A key advantage of this approach is that its interpretation is naturally suited to the problem at hand. Our goal is to assess risks of pre-term delivery resulting from differing blood levels of certain chemicals, namely dde and pcb_*'s, while also differentiating among varying levels of severity of such pre-term births. An ordinal logistic regression model provides probability of response accross various, categories of gestational delivery age, taking into account different severities, while also incorporating the natural ordinality of the different gestational categories. One disadvantage of this model is a relatively stringent structural assumption, namely that of proportional odds. This is a relatively difficult requirement to proof conclusively, though the Brant test, which we have employed, is able to provide support for the assumption. Another disadvantage is the necessity of choosing particular cut points to represent our gestational categories. In one sense, the existence of certain natural cut points in fetal development, as provided by various reputable organizations as WHO, may make this type of categorization seem sensible. On the other hand, categorizing continuous data will always possess a degree of arbitrarity, and it may risk masking different dynamics contained within categories or across categories. An alternative modeling framework which may have been of use is quantile regression. Quantile regression, which remarkably few assumptions, provides an estimation of a given quantile of the response variable as a linear function of predictor variables. One can then fit this model across a range of quantiles to obtain a sense of how the overall distribution of the response changes over the predictor space. This could allow one to identify that, hypothetically, the 20th or 10th quantile of gestational age tends to decrease with increasing DDE concentration, even if the median or higher quantiles are little affected, allowing one to assess the presence of increasing downside risk in the form of longer tails. This also seems promising as a means of answering the question of interest. This method can easily handle to non-normality of our response variable, and it also provides a sense changing likelihood of pre-term births while also differentiating among different levels of severity. It does strike us, however, that there may be some increased difficulty in interpretability, as the true question of interest seems closer to assessing the likelihood of gestational age falling below a given level, as opposed to estimating a given quantile of gestational age. 

We end up concluding that the first principle component is sufficient. As the weights in the first principle component are all of the same sign and of broadly similar magnitude, we could have perhaps simply taken the sum of the pcb_* predictors as an alternative methodology.
